...
class Experiment:
    ...
    def run(self, directory):
        ...
        for i, (train_val_index, test_index) in enumerate(rskf.split(covariates, prepr)):
            ...
            skf_val_ratio = StratifiedShuffleSplit(n_splits=1, train_size=train_ratio, random_state=42)

            prepr_val = experimental_design(y_train_val, amounts_train_val)

            #  ------------------------------------------------------------------------------
            #  Training datasets (train+val) setup for train_ratio  
            for train_index, val_index in skf_val_ratio.split(x_train_val, prepr_val):
            ...
            sparsity_ratio = self.settings['sparsity_ratio']
            print(f'sparsity_ratio : {sparsity_ratio} \n')
            if self.settings['sparsity_ratio'] < 0.0 or self.settings['sparsity_ratio'] >= 1: 
                # Checking that sparsity_ratio included in [0.0 ; 1.0[
                print('\n ------------------------------------------- \n')
                print(f'Error: the sparsity_ratio should always be a value in the [0.0 ; 1.0[ range. The value you selected, {sparsity_ratio}, falls outisde that range. \n')
                print('Please select an appropriate value for sparsity_ratio and restart \n')
                print(' ------------------------------------------- \n \n')
                exit()
            else:
                if self.settings['sparsity_ratio'] != 0.0: 
                    skf_sparsity_ratio = StratifiedShuffleSplit(n_splits=1, train_size=sparsity_ratio, random_state=42) 
                    #prepr_val_clean = experimental_design(y_train_val, amounts_train_val)
                    #  ------------------------------------------------------------------------------
                    #  Training datasets (train+val) setup for sparsity_ratio  
                    for train_index, val_index in skf_sparsity_ratio.split(x_train_val, prepr_val):
                        noisy_x_train, clean_x_val = x_train_val.iloc[train_index], x_train_val.iloc[val_index]
                        noisy_y_train, clean_y_val = y_train_val[train_index], y_train_val[val_index]
                        noisy_cost_matrix, clean_cost_matrix_val = cost_matrix_train_val[train_index, :], cost_matrix_train_val[
                                                                                                    val_index, :]

                        # Setting: instance or class-dependent costs? Sparse, noisy and average
                        if self.settings['sparse_flag']== False:
                            # Standard deviation is a percentage of the original values
                            std_dev = noisy_cost_matrix * (self.settings['noise_level'])
                             # Generate random Gaussian noise with the calculated standard deviation
                            noise = np.random.normal(loc=0, scale=std_dev, size=noisy_cost_matrix.shape)  
                            # Add the noise to the original matrix
                            noisy_cost_matrix = noisy_cost_matrix + noise
                            
                        else: 
                            zero_cost_matrix = np.zeros(cost_matrix_train.shape)
                        class_averaged_noisy_cost_matrix = np.tile(clean_cost_matrix_val.mean(axis=0)[None, :], (len(noisy_y_train), 1, 1)) # based on the average class costs in clean cost matrix  
                    #  ------------------------------------------------------------------------------
                    ...
                    #  ------------------------------------------------------------------------------                    
                    #  Update 24 Jan: separating validation sets for tuning and training
                    #  ------------------------------------------------------------------------------
                    ...
                    # Differentiating validation sets for training and tuning
                    if self.settings['Flag_different_validationSetsForTrainingAndTuning']:
                        tuning_ratio = 0.5
                        skf_val_ratio = StratifiedShuffleSplit(n_splits=1, train_size=tuning_ratio, random_state=42)
                        prepr_val_tuning = experimental_design(y_val, amounts_train_val)
                    """
                     
                    # Preprocessing: Handle missing data, convert categorical variables, standardize, convert to numpy
                    noisy_x_train, clean_x_val, x_test_2, categorical_variables = handle_missing_data(noisy_x_train, clean_x_val, x_test_2,
                                                                                        categorical_variables)
                    
                    noisy_x_train, clean_x_val, x_test_2 = convert_categorical_variables(noisy_x_train, noisy_y_train, clean_x_val, x_test_2,
                                                                        categorical_variables)
                    
                    noisy_x_train, clean_x_val, x_test_2 = standardize(x_train=noisy_x_train, x_val=clean_x_val, x_test=x_test_2)
            ...
            def evaluate_model(model, proba_val, proba, j, index, info):

                thresholds_comparison = self.settings['thresholds_comparison']
                # If one wants to compare different threshold techniques applied in combination with the different training methods, 
                # set the parameter thresholds_comparison to True  
                # By default, it is set to False as the paper focus on comparing different methods in training 
                # (not the different decision making methods)
                if thresholds_comparison:  
                    ...
                    if model.cost_sensitive:
                        isotonic.fit(proba_val, clean_y_val)  # Fit on validation set!
                    else:
                    ...
                    if model.cost_sensitive:
                        threshold_opt_val = empirical_thresholding(proba_val, clean_y_val, clean_cost_matrix_val, metric='idcosts')                    
                    else:
                    ...
                    if model.cost_sensitive:
                        threshold_opt_val = empirical_thresholding(proba_val, clean_y_val, clean_cost_matrix_val, metric='cdcosts')
                    else:
                    ...
                    if model.cost_sensitive:
                        threshold_opt_val = empirical_thresholding(proba_val, clean_y_val, clean_cost_matrix_val, metric='f1')
                    else:
                ...
                # Comparing the training method results with: 
                #  1. Cost-insensitive threshold - Fixed Threshold (0.5)	
                #  2. Raw probabilities: No Thresholding and skip binary decision steps and expected cost based on calculated probabilities)
                #  Actually 2 is not needed as I calcute most of the relevant metrics with 1 --> I removed 2. 
                else: 

                    # 1. Regular Cost-insensitive threshold (0.5):
                    pred = (proba > threshold_cost_ins).astype(int)
                    self.results_tr_insensitive = get_performance_metrics(self.evaluators, self.results_tr_insensitive, j,
                                                                        index, cost_matrix_test, y_test, proba, pred,
                                                                        info)
            ...
            if self.methodologies['logit']:
                print('\n\n***** Logistic regression *****')
                print('\n' + datetime.datetime.now().strftime('Logistic regression at:  %d-%m-%y  |  %H:%M'))
                ...
                print('\n evaluate_model')                
                evaluate_model(logit, logit_proba_val, logit_proba, i, index, info)
            ...
            if self.methodologies['wlogit']:
                print('\n\n*****  Weighted logistic regression *****')
                print('\n' + datetime.datetime.now().strftime('Weighted logistic at:  %d-%m-%y  |  %H:%M'))
                print('\twlogit:')
                try:
                    init_logit
                except NameError:
                ...
                evaluate_model(wlogit, wlogit_proba_val, wlogit_proba, i, index, info)
            ...
            if self.methodologies['cslogit']:
                print('\n\n*****  Cost-sensitive logistic regression *****')
                print('\n' + datetime.datetime.now().strftime('Cost-sensitive logistic regression at:  %d-%m-%y  |  %H:%M'))
                print('\tcslogit:')
                try:
                    init_logit
                except NameError:
                ...
                evaluate_model(cslogit, cslogit_proba_val, cslogit_proba, i, index, info)
            ...
            if self.methodologies['net']:
                print('\n\n*****  Neural network *****')
                print('\n' + datetime.datetime.now().strftime('Net at:  %d-%m-%y  |  %H:%M'))
                print('\tneural network:')

                neural_network = CSNeuralNetwork(n_inputs=x_train.shape[1], obj='ce', training_cost_matrix_available = True)
                ...
                evaluate_model(neural_network, nn_proba_val, nn_proba, i, index, info)

                index += 1

            if self.methodologies['wnet']:
                print('\n\n*****  wnet *****')
                print('\n' + datetime.datetime.now().strftime('wnet at:  %d-%m-%y  |  %H:%M'))
                print('\twnet:')

                wnet = CSNeuralNetwork(n_inputs=x_train.shape[1], obj='weightedce', directory=directory, training_cost_matrix_available = True)

                # Not sure it is a good idea to change the tune function here
                # --> Potential question for Yoann  
                wnet = wnet.tune(self.l1, self.lambda1_list, self.l2, self.lambda2_list, self.neurons_list, noisy_x_train,
                                 noisy_y_train, class_averaged_noisy_cost_matrix, clean_x_val, clean_y_val, clean_cost_matrix_val)

                lambda1 = wnet.lambda1
                lambda2 = wnet.lambda2

                start = time.perf_counter()
                wnet = wnet.model_train(wnet, noisy_x_train, noisy_y_train, clean_x_val, clean_y_val,
                                        cost_matrix_train=class_averaged_noisy_cost_matrix, cost_matrix_val=clean_cost_matrix_val)
                end = time.perf_counter()

                wnet_proba = wnet.model_predict(wnet, x_test)
                wnet_proba_val = wnet.model_predict(wnet, clean_x_val)

                info = {'time': end - start, 'lambda1': lambda1, 'lambda2': lambda2,
                        'n_neurons': wnet.lin_layer1.out_features}

                evaluate_model(wnet, wnet_proba_val, wnet_proba, i, index, info)

                index += 1

            if self.methodologies['csnet']:
                print('\n\n*****  csnet *****')
                print('\n' + datetime.datetime.now().strftime('csnet at:  %d-%m-%y  |  %H:%M'))
                print('\tcsnet:')

                csnet = CSNeuralNetwork(n_inputs=x_train.shape[1], obj='aec', directory=directory, training_cost_matrix_available = True)

                csnet = csnet.tune(self.l1, self.lambda1_list, self.l2, self.lambda2_list, self.neurons_list, noisy_x_train,
                                 noisy_y_train, class_averaged_noisy_cost_matrix, clean_x_val, clean_y_val, clean_cost_matrix_val) 
                ...
                start = time.perf_counter()
                csnet = csnet.model_train(csnet, noisy_x_train, noisy_y_train, clean_x_val, clean_y_val,
                                        cost_matrix_train=class_averaged_noisy_cost_matrix, cost_matrix_val=clean_cost_matrix_val) 
                end = time.perf_counter()

                csnet_proba = csnet.model_predict(csnet, x_test)
                csnet_proba_val = csnet.model_predict(csnet, clean_x_val)

                info = {'time': end - start, 'lambda1': lambda1, 'lambda2': lambda2,
                        'n_neurons': csnet.lin_layer1.out_features}

                evaluate_model(csnet, csnet_proba_val, csnet_proba, i, index, info)

                index += 1

            if self.methodologies['noisy_wnet']:
                print('\n\n*****  noisy_wnet *****')
                print('\n' + datetime.datetime.now().strftime('noisy_wnet at:  %d-%m-%y  |  %H:%M'))
                print('\tnoisy_wnet:')

                noisy_wnet = CSNeuralNetwork(n_inputs=x_train.shape[1], obj='weightedce', directory=directory, training_cost_matrix_available = True)

                noisy_wnet = noisy_wnet.tune(self.l1, self.lambda1_list, self.l2, self.lambda2_list, self.neurons_list, noisy_x_train,
                                 noisy_y_train, noisy_cost_matrix, clean_x_val, clean_y_val, clean_cost_matrix_val)

                lambda1 = noisy_wnet.lambda1
                lambda2 = noisy_wnet.lambda2

                start = time.perf_counter()
                noisy_wnet = noisy_wnet.model_train(noisy_wnet, noisy_x_train, noisy_y_train, clean_x_val, clean_y_val,
                                        cost_matrix_train=noisy_cost_matrix, cost_matrix_val=clean_cost_matrix_val)
                end = time.perf_counter()

                noisy_wnet_proba = noisy_wnet.model_predict(noisy_wnet, x_test)
                noisy_wnet_proba_val = noisy_wnet.model_predict(noisy_wnet, clean_x_val)

                info = {'time': end - start, 'lambda1': lambda1, 'lambda2': lambda2,
                        'n_neurons': noisy_wnet.lin_layer1.out_features}

                evaluate_model(noisy_wnet, noisy_wnet_proba_val, noisy_wnet_proba, i, index, info)

                index += 1

            if self.methodologies['noisy_csnet']:
                print('\n\n*****  noisy_csnet *****')
                print('\n' + datetime.datetime.now().strftime('noisy_csnet at:  %d-%m-%y  |  %H:%M'))
                print('\tnoisy_csnet:')

                noisy_csnet = CSNeuralNetwork(n_inputs=x_train.shape[1], obj='aec', directory=directory, training_cost_matrix_available = True)

                noisy_csnet = noisy_csnet.tune(self.l1, self.lambda1_list, self.l2, self.lambda2_list, self.neurons_list, noisy_x_train,
                                 noisy_y_train, noisy_cost_matrix, clean_x_val, clean_y_val, clean_cost_matrix_val) 

                lambda1 = noisy_csnet.lambda1
                lambda2 = noisy_csnet.lambda2

                start = time.perf_counter()
                noisy_csnet = noisy_csnet.model_train(noisy_csnet, noisy_x_train, noisy_y_train, clean_x_val, clean_y_val,
                                        cost_matrix_train=noisy_cost_matrix, cost_matrix_val=clean_cost_matrix_val) 
                end = time.perf_counter()

                noisy_csnet_proba = noisy_csnet.model_predict(noisy_csnet, x_test)
                noisy_csnet_proba_val = noisy_csnet.model_predict(noisy_csnet, clean_x_val)

                info = {'time': end - start, 'lambda1': lambda1, 'lambda2': lambda2,
                        'n_neurons': noisy_csnet.lin_layer1.out_features}

                evaluate_model(noisy_csnet, noisy_csnet_proba_val, noisy_csnet_proba, i, index, info)

                index += 1

            if self.methodologies['sparsity_resilient_csnet']: #sparo_csnet Sparsity Robust -  sparse cost (robust) net 
                print('\n\n*****  sparsity_resilient_csnet  *****')
                print('\n' + datetime.datetime.now().strftime('sparsity_resilient_csnet at:  %d-%m-%y  |  %H:%M'))
                print('\tsparsity_resilient_csnet:')

                #  Net with Sparse_noisy_costs_reweight_autodiff with sparse cost matrix

            
                sparo_csnet = CSNeuralNetwork(n_inputs=x_train.shape[1], obj='reweighting_method', directory=directory, training_cost_matrix_available = False)

                sparo_csnet = sparo_csnet.tune(self.l1, self.lambda1_list, self.l2, self.lambda2_list, self.neurons_list, noisy_x_train,
                                 noisy_y_train, None, clean_x_val, clean_y_val, clean_cost_matrix_val) 

                lambda1 = sparo_csnet.lambda1
                lambda2 = sparo_csnet.lambda2

                start = time.perf_counter()
                sparo_csnet = sparo_csnet.model_train(sparo_csnet, noisy_x_train, noisy_y_train, clean_x_val, clean_y_val,
                                        cost_matrix_train=None, cost_matrix_val=clean_cost_matrix_val)
                end = time.perf_counter()

                sparo_csnet_proba = sparo_csnet.model_predict(sparo_csnet, x_test)
                sparo_csnet_proba_val = sparo_csnet.model_predict(sparo_csnet, clean_x_val)

                info = {'time': end - start, 'lambda1': lambda1, 'lambda2': lambda2,
                        'n_neurons': sparo_csnet.lin_layer1.out_features}

                evaluate_model(sparo_csnet, sparo_csnet_proba_val, sparo_csnet_proba, i, index, info)

                index += 1

            if self.methodologies['noise_resilient_csnet']: #sparo_csnet Sparsity Robust -  sparse cost (robust) net 
                print('\n\n*****  noise_resilient_csnet  *****')
                print('\n' + datetime.datetime.now().strftime('noise_resilient_csnet at:  %d-%m-%y  |  %H:%M'))
                print('\tnoise_resilient_csnet:')

                # Net with Sparse_noisy_costs_reweight_autodiff with noisy cost matrix 

                sparo_csnet = CSNeuralNetwork(n_inputs=x_train.shape[1], obj='reweighting_method', directory=directory, training_cost_matrix_available = True)
            ...
            if self.methodologies['boost']:
                print('\n\n*****  boost *****')
                print('\n' + datetime.datetime.now().strftime('boost at:  %d-%m-%y  |  %H:%M'))
                print('\txgboost:')
            ...
            if self.methodologies['wboost']:
                print('\n\n*****  wboost *****')
                print('\n' + datetime.datetime.now().strftime('wboost at:  %d-%m-%y  |  %H:%M'))
                print('\twboost:')
            ...
            if self.methodologies['csboost']:
                print('\n\n*****  csboost *****')
                print('\n' + datetime.datetime.now().strftime('csboost at:  %d-%m-%y  |  %H:%M'))
                print('\tcsboost:')
