...
def evaluate_experiments(evaluators, methodologies, evaluation_matrices, directory, name):
    ...
    if evaluators['savings']:
        ...
        if n_methodologies > 1: # Change due to issue with Orange3
            cd = compute_CD(avg_rankings, n=1, alpha='0.05', test="nemenyi") #### Here 
            print(f'Critical difference: {np.round(cd, 4)}')
            graph_ranks(avg_rankings, names, cd=cd, width=9, textspace=3, lowv=1, highv=n_methodologies)  #### Here 
            # plt.show()
